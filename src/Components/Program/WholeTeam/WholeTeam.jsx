...
export default function WholeTeam() {
  ...
  return (
    ...
    <div className='wholeTeam_section'>
        {
            data.map((data, idx)=>(
                <TeamCard key={idx} image={data.image} title={data.title} desc={data.description}/>
            ))
        }
    </div>
    <button className='wholeTeam_btn' onClick={()=>navigate('/#')}>Go Back</button>
    </>
  )
}
